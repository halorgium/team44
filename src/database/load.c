...
int loadDatabase()
{
    ...
    if (InFile == NULL)
        return NEXTID_LOAD_FAILURE;
    if (loadNextID(InFile) != 1)
        return NEXTID_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return USER_LOAD_FAILURE;
    if (loadAllUsers(InFile) != 1)
        return USER_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return ALBUM_LOAD_FAILURE;
    if (loadAllAlbums(InFile) != 1)
        return ALBUM_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return ARTIST_LOAD_FAILURE;
    if (loadAllArtists(InFile) != 1)
        return ARTIST_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return USR_COM_LOAD_FAILURE;
    if (loadUserComments(InFile) != 1)
        return USR_COM_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return ALB_COM_LOAD_FAILURE;
    if (loadAlbumComments(InFile) != 1)
        return ALB_COM_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return ART_COM_LOAD_FAILURE;
    if (loadArtistComments(InFile) != 1)
        return ART_COM_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return LOAN_LOAD_FAILURE;
    if (loadAllLoans(InFile) != 1)
        return LOAN_LOAD_FAILURE;
    fclose(InFile);
    ...
    if (InFile == NULL)
        return LOANRET_LOAD_FAILURE;
    if (loadAllLoansReturned(InFile) != 1)
        return LOANRET_LOAD_FAILURE;
    fclose(InFile);
...
static int loadAllUsers(FILE * file)
{
    ...
    while ((line = readLine(file)) != NULL) {
        ...
        /* get userCode */
        newUser->userCode =
            malloc(sizeof(char) * (strlen(temp) - strlen(temp2)) + 1);
...
static int loadUserComments(FILE * file)
{
    ...
    while ((line = readLine(file)) != NULL) {
        ...
        strncpy(newUserComment->comment, temp,
...
static int loadAlbumComments(FILE * file)
{
    ...
    while ((line = readLine(file)) != NULL) {
        ...
        strncpy(newAlbumComment->comment, temp,
                (strlen(temp) - strlen(temp2)));
        /*null terminate new string */
...
static int loadArtistComments(FILE * file)
{
    ...
    while ((line = readLine(file)) != NULL) {
        ...
        strncpy(newArtistComment->comment, temp,
                (strlen(temp) - strlen(temp2)));
        /*null terminate new string */
